...
const ResultsComponent = () => {
  ...
  const [selectedDisciplineGroup, setSelectedDisciplineGroup] =
    useState<string>("");
  const [selectedDisciplinesList, setSelectedDisciplinesList] = useState<
    GenericStringIndex[]
  >([]);
  ...
  const [categoryMappingId, setCategoryMappingId] =
    useState<CategoryMappingIdType>({});
  ...
  useEffect(() => {
    const getData = async () => {
      const selectedDisciplinesId: number[] = selectedDisciplinesList.map(
        (disc) => Number(disc.id),
      );
  ...
  useEffect(() => {
    ...
    if (results.length) {
      ...
      results.forEach((result) => {
        ...
        tableByCategory[categoryId as keyof typeof tableByCategory]?.push(
          result,
        );
      });
