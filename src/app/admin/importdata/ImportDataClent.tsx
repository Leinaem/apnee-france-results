...
interface ImportDataClentProps {
  competitionList: GenericStringIndexWithDate[];
  disciplineList: GenericStringIndexWithDate[];
}

const ImportDataClent = ({competitionList, disciplineList}: ImportDataClentProps) => {
  ...
  const prepareData = (data: GenericStringIndex[]) => {
    ...
    if (selectedTable === "results" && disciplineList.length) {
      const competitionId = selectedCompetition;
      data.forEach((item: GenericStringIndex) => {
        const disciplineId = disciplineList.find(
          (disc) =>
          ...
          {selectedTable === "results" && Boolean(competitionList.length) && (
            <>
              <InputSelect
                ...
                options={competitionList as GenericStringIndex[]}
...
export default ImportDataClent;
